...
void cmap_to_fb(uint8_t *out, uint8_t *in, int in_pixels)
{
    ...
    for (i = 0; i < in_pixels; i++)
    {
        ...
        {
            /* ARGB8888 packing: enforce full opacity in alpha channel */
            pix = (0xFFu << s_Fb.transp.offset) |
                  ((uint32_t)c.r << s_Fb.red.offset) |
                  ((uint32_t)c.g << s_Fb.green.offset) |
                  ((uint32_t)c.b << s_Fb.blue.offset);
