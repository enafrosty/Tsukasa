...
void Z_Free (void* ptr)
{
    ...
    if (block->id != ZONEID)
	I_Error ("Z_Free: freed a pointer without ZONEID (ptr=%p block=%p id=0x%x tag=%d size=%d expected=0x%x)",
                 ptr, block, (unsigned int)block->id, block->tag, block->size, (unsigned int)ZONEID);
